--Napisane przez V0ID'a na licencji CC-BY-SA (użycie zezwolone tylko poprzez uwzględnienie autora w tekście w "OnPlayerConnect")
local Player = {};
...
for i = 0, GetMaxPlayers() - 1 do
	...
	Player[i].class = 0;
	Player[i].moderator = 0; --Jest moderatorałkę?
...
function OnPlayerConnect(playerid)
	SendPlayerMessage(playerid, 0, 255, 0, "Gamemode stworzony przez V0ID'a, nieznacznie zmodyfikowany przez Czytaj Noł Fejs'a.");
	SendPlayerMessage(playerid, 0, 255, 0, "Pamiętaj, że Czytaj Noł Fejs nie jest (i nigdy nie będzie) członkiem ekipy tego serwera.");
	SendPlayerMessage(playerid, 255, 145, 0, "Witaj na serwerze Potęga Koloni RolePlay!");
	SendPlayerMessage(playerid, 255, 145, 0, "Wpisz /pomoc aby dowiedzieć się jak grać na serwerze!");
	
	SendPlayerMessage(playerid, 0, 255, 0, "Wpisz /n twoja_nazwa aby ustawić nazwę postaci");
	...
	local msgType = GetChatMessageType(text);
		if msgType == "RP" then
			for i = 0, GetMaxPlayers() - 1 do
				if IsNPC(i) == 0 then
					if i == playerid then
						SendPlayerMessage(i, 255, 255, 255, string.format("%s %s %s %s %s %s","(", playerid, ")", GetPlayerName(playerid), "mówi:", text));
					else
					if GetPlayerWorld(playerid) == GetPlayerWorld(i) then
						if GetDistancePlayers(playerid, i) < 1000 then								
							SendPlayerMessage(i, 230, 230, 230, string.format("%s %s %s %s %s %s","(", playerid, ")", GetPlayerName(playerid), "mówi:", text));
						elseif GetDistancePlayers(playerid, i) > 1000 and GetDistancePlayers(playerid, i) < 1250 then
							SendPlayerMessage(i, 220, 220, 220, string.format("%s %s", "Ktoś w oddali mówi:", text));
						end
		...
		SendPlayerMessage(playerid, 255, 0, 0, "Zaloguj się aby móc pisać");
	end
end
...
function OnPlayerDisconnect(playerid, reason)
	...
	if Player[playerid].logged == true then
		...
		SendMessageToAll(255, 0, 0, string.format("%s %s", GetPlayerName(playerid), "odłączył(a) się od gry"));
	end
	...
	if Player[playerid].logged == false then
		for i = 0, GetMaxPlayers() -1 do
			if IsPlayerConnected(i) == 1 then
				if i ~= playerid then
					SendPlayerMessage(i, 0, 255, 0, string.format("%s %s", GetPlayerName(playerid), "dołączył(a) do gry!"));
				end
			end	
		end
	...
	if IsNPC(playerid) == 0 then
		if Player[playerid].class == 0 then
			if weaponmode ~= WEAPON_NONE then
				SendPlayerMessage(playerid, 255, 0, 0, "Jesteś zbyt słaby żeby walczyć")
...
function CMD_Register(playerid, cmd, params)
	...
	if result == 1 then
		if Player[playerid].logged == false then
			...
			if fRead then
				SendPlayerMessage(playerid, 255, 0, 0, "Posiadasz już konto na serwerze. Wpisz /zaloguj hasło aby się zalogować")
				fRead:close()
			else
				...
				SendPlayerMessage(playerid, 179, 0, 255, "Zarejestrowano pomyślnie, życzymy dobrej zabawy")
				GameTextForPlayer(playerid, 1500, 7000, "Zapraszamy na naszą stronę http://potegakhorinis.y0.pl/", "Font_Old_10_White_Hi.TGA", 0, 255, 255, 3000)
				SetPlayerColor(playerid, 255, 255, 255)
				Class_NewPlayer(playerid)
				SaveAccount(playerid)
			end
		else
			SendPlayerMessage(playerid, 255, 0, 0, "Jesteś już zalogowany!")
		end
	else
		SendPlayerMessage(playerid, 255, 0, 0, "Błędna składnia komendy wpisz /zarejestruj hasło");
	end
end
--Logowanie
			...
			if fRead then
				...
				local result, passwd = sscanf(tmp, "s")
					if pass == passwd then
						...
						SendPlayerMessage(playerid, 0, 255, 154, "Zostałeś zalogowany")
						GameTextForPlayer(playerid, 1500, 7000, "Zapraszamy na naszą stronę http://potegakhorinis.y0.pl/", "Font_Old_10_White_Hi.TGA", 0, 255, 255, 3000)
						SetPlayerColor(playerid, 255, 255, 255)
					else
						SendPlayerMessage(playerid, 255, 0, 0, "Błędne hasło! Podaj te, podane podczas rejestracji.")
						SendPlayerMessage(playerid, 255, 0, 0, "Jeżeli nie pamiętasz hasła, powiadom administratora serwera.")
					end
			else
				SendPlayerMessage(playerid, 255, 0, 0, "Nie masz konta na serwerze wpisz /zarejestruj hasło")
			end
		else
			SendPlayerMessage(playerid, 255, 0, 0, "Jesteś już zalogowany!")
		end
	else
		SendPlayerMessage(playerid, 255, 0, 0, "Błędna składnia komendy wpisz /zaloguj hasło");
	end
end

function CMD_GMSG(playerid, cmd, params) --Wiadomość do gildii
	local result, message = sscanf(params, "s")
	if result == 1 then --205, 133, 63
		if Player[playerid].guild == -1 then
			SendPlayerMessage(playerid, 255, 0, 0, "Nie należysz do żadnej oficjalnej gildii.")
		else
			for i = 0, GetMaxPlayers()-1 do
				if IsPlayerConnected(i) == 1 then
		...
		SendPlayerMessage(playerid, 255, 0, 0, "Użyj /gmsg wiadomość")
	end
end
						...
						SendPlayerMessage(playerid, 255, 68, 0, "(PM) "..GetPlayerName(id).."|"..id.." << "..msg)
							...
							for i = 0, GetMaxPlayers() - 1 do -- Info do adma
								if IsPlayerAdmin(i) == 1 and i ~= playerid and i ~= id and Player[i].seepm == true then
									SendPlayerMessage(i, 0, 255, 94, "Podgląd (PM)| od "..GetPlayerName(playerid).."|"..playerid.." do "..GetPlayerName(id).."|"..id..": "..msg)
								end
							end
				else
					SendPlayerMessage(playerid, 255, 0, 0, "Ten gracz nie jest połączony, lub piszesz do siebie.")
				end
		else
			SendPlayerMessage(playerid, 255, 0, 0, "Użyj: /pm id wiadomosc")
		end
end
	...
	local headModel = { "Hum_Head_FatBald", "Hum_Head_Fighter", "Hum_Head_Pony", "Hum_Head_Bald", "Hum_Head_Thief", "Hum_Head_Psionic", "Hum_Head_Babe" }
		local result, body, bodytex, head, headtex = sscanf(params, "dddd")
			if result == 1 then
				...
				SendPlayerMessage(playerid, 0, 255, 0, "Wygląd zmieniony!")
			else
				SendPlayerMessage(playerid, 255, 0, 0, "Użyj /wyglad modelCiała(1-2), teksturaCiała(0-12), modelGłowy(1-7), teksturaGłowy(0-162)")
			end
end
...
function CMD_Help(playerid, cmd, params)
	...
	if page == 1 then
		SendPlayerMessage(playerid, 255, 255, 0, "Komendy serwera Potęga Kolonii RP");
		SendPlayerMessage(playerid, 255, 255, 0, "Zapoznaj się z nowym czatem rp opartym na prefix'ach:");
		SendPlayerMessage(playerid, 255, 255, 0, "@ - Wiadomość zostanie wyświetlona jako ooc ");
		SendPlayerMessage(playerid, 255, 255, 0, ". - Wiadomość zostanie wyświetlona jako do ");
		SendPlayerMessage(playerid, 255, 255, 0, "! - Wiadomość zostanie wyświetlona jako krzyk ");
		SendPlayerMessage(playerid, 255, 255, 0, "# - Wiadomość zostanie wyświetlona jako me(ja) ");
		SendPlayerMessage(playerid, 255, 255, 0, "Dalsza pomoc: /pomoc 2 ");
	elseif page == 2 then
		SendPlayerMessage(playerid, 255, 255, 0, "Komendy serwera Potęga Kolonii RP");
		SendPlayerMessage(playerid, 255, 255, 0, "/pm id/nick_gracza - Prywatna wiadomość");
		SendPlayerMessage(playerid, 255, 255, 0, "/adm wiadomość - Wiadomość do administratora (ujrzą ją wszyscy administratorzy online)");
		SendPlayerMessage(playerid, 255, 255, 0, "/wyglad modelCiała(1-2) teksturaCiała(0-12) modelGłowy(1-7) teksturaGłowy(0-162) - Zmiana wyglądu");
		SendPlayerMessage(playerid, 255, 255, 0, "/gmsg wiadomość - Wiadomość do gildii");
		SendPlayerMessage(playerid, 255, 255, 0, "/zmienhaslo stareHasło noweHasło - zmiana hasła");
...
function CMD_MHelp(playerid, cmd, params)
	...
	if IsPlayerAdmin(playerid) == 1 or Player[playerid].moderator == 1 then
		if page == 1 then
			SendPlayerMessage(playerid, 255, 255, 0, "Komendy moderatora na serwerzer Potęga Kolonii eRPe.")
			SendPlayerMessage(playerid, 255, 255, 0, "/awans idgracza idklasy - Zmiana klasy")
			SendPlayerMessage(playerid, 255, 255, 0, "/post wiadomość - Wiadomość globalna")
			SendPlayerMessage(playerid, 255, 255, 0, "/m.kick idgracza powód - Wyrzucenie danego gracza z serwera")
			SendPlayerMessage(playerid, 255, 255, 0, "/m.all - Wyświetla moderatorów oraz administratorów online")
		else
			SendPlayerMessage(playerid, 255, 255, 0, "Komendy moderatora na serwerzer Potęga Kolonii")
			SendPlayerMessage(playerid, 255, 255, 0, "/awans idgracza idklasy - Zmiana klasy")
			SendPlayerMessage(playerid, 255, 255, 0, "/post wiadomość - Wiadomość globalna")
			SendPlayerMessage(playerid, 255, 255, 0, "/m.kick idgracza powód - Wyrzucenie danego gracza z serwera")
			SendPlayerMessage(playerid, 255, 255, 0, "/m.all - Wyświetla moderatorów oraz administratorów online")
		...
		SendPlayerMessage(playerid, 255, 0, 0, "Nie masz uprawnień do użycia tej komendy")
	end
end
...
function CMD_MKick(playerid, cmd, params)
	if IsPlayerAdmin(playerid) == 1 or Player[playerid].moderator == 1 then
		local result, id, reason = sscanf(params, "ds")
			if result == 1 then
				SendMessageToAll(255, 0, 0, "Gracz "..GetPlayerName(id).." został wyrzucony z serwera przez moderatora "..GetPlayerName(playerid).." powód: "..reason)
				Kick(id)
			else
				SendPlayerMessage(playerid, 255, 0, 0, "Użyj /m.kick idgracza powód")
			end
	else
		SendPlayerMessage(playerid, 255, 0, 0, "Nie masz uprawnień do użycia tej komendy")
	end
...
function CMD_MAll(playerid, cmd, params)
	if IsPlayerAdmin(playerid) == 1 or IsPlayerLeader(playerid) == 1 or Player[playerid].moderator == 1 then
		SendPlayerMessage(playerid, 255, 255, 0, "Osoby z uprawnienami przebywające obecnie na serwerze:")
		for i = 0, GetMaxPlayers() -1 do
			if IsPlayerConnected(i) == 1 then
...
function CMD_Post(playerid, cmd, params) --Global
	if IsPlayerAdmin(playerid) == 1 or Player[playerid].moderator == 1 or IsPlayerLeader(playerid) == 1 then
		...
		if result == 1 then
			...
			SendPlayerMessage(playerid, 255, 0, 0, "Użyj /post wiadomość")
		end
	else
		SendPlayerMessage(playerid, 255, 0, 0, "Nie masz uprawnień do tej komendy");
	end
end
		...
		if IsPlayerAdmin(playerid) == 1 then
			for i, v in ipairs(Classes) do
				if v.classid == class then
					SendPlayerMessage(id, 0, 255, 0, "Twoja klasa została zamieniona na: "..v.name)
					SendPlayerMessage(playerid, 0, 255, 0, "Klasa gracza "..GetPlayerName(id).." została zamieniona na:"..v.name)
					ClearInventory(id)
					v.func(id)
				end
			end
			...
			if Player[playerid].moderator == 1 or IsPlayerLeader(playerid) == 1 then
				...
				SendPlayerMessage(playerid, 255, 0, 0, "Nie jesteś administratorem, ani dowódcą!")
			end
		end
	else
		SendPlayerMessage(playerid, 255, 0, 0, "Użyj /awans idgracza idklasy")
	end
end
		...
		for i = 0, GetMaxPlayers() -1 do
			if playerid ~= i then
				if IsPlayerAdmin(i) == 1 or Player[i].moderator == 1 then
					SendPlayerMessage(i, 0, 255, 255, "Wiadomość dla administratora:");
					SendPlayerMessage(i, 0, 255, 255, "(Adm)"..playerid.."|"..GetPlayerName(playerid)..":"..msg);
				end
			end
		end
		SendPlayerMessage(playerid, 0, 255, 0, "Wiadomość została wysłana.");
	end
end
	...
	if result == 1 then
		if Player[playerid].logged == true then
			if oldPass == Player[playerid].password then
				...
				SendPlayerMessage(playerid, 0, 255, 0, "Hasło zostało zmienione")
				SaveAccount(playerid)
			else
				SendPlayerMessage(playerid, 255, 0, 0, "Stare hasło nie pasuje do używanego obecnie")
			end
		else
			SendPlayerMessage(playerid, 255, 0, 0, "Musisz być zalogowany żeby zmienić hasło")
		end
	else
		SendPlayerMessage(playerid, 255, 0, 0, "Użyj /zmienhaslo stareHasło noweHasło")
	end
end
	...
	if IsPlayerAdmin(playerid) == 1 then
		...
		if result == 1 then
			if Player[id].moderator == 0 then
				SendPlayerMessage(playerid, 0, 255, 0, "Nadałeś uprawnienia moderatora graczowi "..GetPlayerName(id).."|"..id)
				SendPlayerMessage(id, 0, 255, 0, "Administrator "..GetPlayerName(playerid).."|"..playerid.." nadał Ci uprawnienia moderatora")
				Player[id].moderator = 1
			else
				SendPlayerMessage(playerid, 255, 0, 0, "Zabrałeś uprawnienia moderatora graczowi "..GetPlayerName(id).."|"..id)
				SendPlayerMessage(id, 0, 255, 0, "Administrator "..GetPlayerName(playerid).."|"..playerid.." odebrał Ci uprawnienia moderatora")
				Player[id].moderator = 0
			end
		end
		else 
			SendPlayerMEssage(playerid, 255, 0, 0, "Użyj /mod idgracza")
	end
end
	...
	if Player[playerid].logged == false then
		...
		if result == 1 then
			...
			SendPlayerMessage(playerid, 119, 255, 0, "Twoja nazwa postaci: "..nick)
				...
				local fRead = io.open(GetPlayerName(playerid)..".acnt", "r+")
				if fRead then
					SendPlayerMessage(playerid, 111, 255, 0, "Masz już konto na serwerze: wpisz /zaloguj hasło")
					fRead:close()
				else
					SendPlayerMessage(playerid, 239, 255, 0, "Nie masz konta na serwerze: wpisz /zarejestruj hasło")
				end
				--szit dałn wtf?!
...
function CMD_SEEPM(playerid, cmd, params) --Podgląd pm
	if IsPlayerAdmin(playerid) == 1 then
		if Player[playerid].seepm == true then
			Player[playerid].seepm = false
			SendPlayerMessage(playerid, 255, 0, 0, "Podgląd prywatnych wiadomości wyłączony")
		else
			Player[playerid].seepm = true
			SendPlayerMessage(playerid, 0, 255, 0, "Podgląd prywatnych wiadomości włączony")
		end
	end
end
...
function Timer_All() --Zapis, zmiana koloru drejwu itp
	...
	UpdateDraw(GetDraw("server_name"), 5200, 7500, "Potęga Kolonii RolePlay", "Font_Old_20_White_Hi.TGA", colors[color_id].r, colors[color_id].g, colors[color_id].b)
end
...
AddDraw(CreateDraw(5200, 7500, "Potęga Kolonii RolePlay", "Font_Old_20_White_Hi.TGA", 255, 143, 0), "server_name")
...
function Class_CityGuard(playerid) --Strażnik miejski (podstawowy)
...
function Class_CityGuard2(playerid) --Strażnik miejski (zaawansowany)
...
function Class_MedSLD(playerid) --Środkowy najemnik xD
...
function Class_SLD(playerid) --Doświadczony najemnik (taki koks i wgle)
...
function Class_Farm(playerid) --Wieśniak
...
function Class_Gov(playerid) --Urzędnik
...
function Class_Judge(playerid) --Sędzia
...
function Class_BDTL(playerid) --Słaby bandyta
...
function Class_BDTH(playerid) --Bandyta (strażnik)
...
function Class_BOWM(playerid) --Łuczarz
...
function Class_BlackNewb(playerid) --Pachołek cienia
...
function Class_BlackMemb(playerid) --Członek bractwa
...
function Class_Nigger(playerid) --Cień
...
function Class_Pedofil(playerid) --Kapłan (dowódca)
...
RegisterClass(1, Class_CityGuard, "Strażnik miejski")
RegisterClass(2, Class_CityGuard2, "Doświadczony strażnik miejski")
...
RegisterClass(7, Class_FireMage2, "Członek kręgu ognia")
RegisterClass(8, Class_NewSLD, "Najemnik")
RegisterClass(9, Class_MedSLD, "Uzbrojony najemnik")
RegisterClass(10, Class_SLD, "Doświadczony najemnik")
RegisterClass(11, Class_Lord, "Lord miasta Khorinis")
RegisterClass(12, Class_Farm, "Łowca smoków")
RegisterClass(13, Class_Gov, "Urzędnik")
...
RegisterClass(18, Class_Judge, "Sędzia")
RegisterClass(19, Class_BDTL, "Cienias")
RegisterClass(20, Class_BDTM, "Bandyta")
RegisterClass(21, Class_BDTH, "Bandyta (strażnik)")
RegisterClass(22, Class_MASTERBDT, "Herszt bandytów")
RegisterClass(23, Class_V0ID, "Król (V0ID)")
RegisterClass(24, Class_BOWM, "Łuczarz")
RegisterClass(25, Class_General, "Generał")
RegisterClass(26, Class_BlackNewb, "Pachołek cienia")
RegisterClass(27, Class_BlackMemb, "Członek bractwa")
RegisterClass(28, Class_BlackWarrior, "Wojownik mroku")
RegisterClass(29, Class_Nekrofil, "Nekromanta")
RegisterClass(30, Class_Nigger, "Cień")
RegisterClass(31, Class_Pedofil, "Kapłan")
RegisterClass(32, Class_LordKossak, "Lord Kossak")
RegisterClass(33, Class_RekaKossak, "Prawa ręka (Kossak)")
RegisterClass(34, Class_ZolnierzKossak, "Żołnierz (Kossak)")
RegisterClass(35, Class_PracownikKossak, "Farmer (Kossak)")
RegisterClass(36, Class_LordRandall, "Lord Randall")
RegisterClass(37, Class_RekaRandall, "Prawa ręka (Randall)")
RegisterClass(38, Class_ZolnierzRandall, "Żołnierz (Randall)")
